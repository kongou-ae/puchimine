...
def load_project_member(project_id):
    ...
    url = redmine_url + 'projects/' + project_id + '/memberships.json'
    r = requests.get(url,headers={'Content-Type': 'application/json','X-Redmine-API-Key': redmine_api_key})
    data = json.loads(r.text)
...
def load_ticket_detail_summary(ticlet_id):
    ...
    url = redmine_url + 'issues/' + ticlet_id + '.json?include=journals'
    r = requests.get(url,headers={'Content-Type': 'application/json','X-Redmine-API-Key': redmine_api_key})
    data = json.loads(r.text)
...
def update_journal(issue_id,journal_update):
    '''
    ...
    r = requests.put(url,headers={'Content-Type': 'application/json','X-Redmine-API-Key': redmine_api_key},data=data)
